...
namespace Flash.Extensions.Resilience.Http
{
    public class ResilientHttpClientFactory : IHttpClientFactory
    {
        ...
        public IHttpClient CreateResilientHttpClient()
            => new ResilientHttpClient((origin) => CreatePolicies(origin), _logger, _httpContextAccessor, new HttpUrlResolver(_serviceLocator), _tracerFactory);

        public IHttpClient CreateResilientHttpClient(HttpMessageHandler httpMessageHandler)
       => new ResilientHttpClient((origin) => CreatePolicies(origin), _logger, _httpContextAccessor, new HttpUrlResolver(_serviceLocator), httpMessageHandler, _tracerFactory);
