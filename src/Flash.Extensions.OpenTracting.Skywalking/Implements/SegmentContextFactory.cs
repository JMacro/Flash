...
namespace Flash.Extensions.Tracting.Skywalking
{
    public class SegmentContextFactory : ISegmentContextFactory
    {
        ...
        {
            return carrier.HasValue ? (carrier.TraceId > 0 ? carrier.TraceId : _uniqueIdGenerator.NewId()) : _uniqueIdGenerator.NewId();
        }

        private long GetTraceId(SegmentContext parentSegmentContext)
        {
            return parentSegmentContext?.TraceId ?? _uniqueIdGenerator.NewId();
        }
